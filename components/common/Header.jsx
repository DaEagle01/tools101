...
import { MenuIconSVGComponent, MenuSvgComponent } from "../icons/icons";
import { debounce } from "../../utils/debounce";
import { SearchIcon, MicrophoneIcon, ImageIcon, ConfigureIcon, UserOutlinedIcon, UserProfileIcon, HamburgerMenuIcon, CategoryIcon } from "../../components/icons/icons";
...
function Header() {
  ...
  const [searchProduct, { data: searchProductData, error: searchProductError, isLoading: isSearchProductLoading }] = useTextSearchMutation();
  const {
    data: cate_data,
    error,
    isLoading,
  ...
  const handleFindClick = async () => {
    ...
    return router.push(
      `/search-result?category=${selectedValue1?.category_name ?? ""
      }&category_id=${formValue.category ?? ""}&brand=${selectedValue2?.brand_name ?? ""}&brand_id=${formValue.brand ?? ""}&model=${selectedValue3?.model_name ?? ""}&model_id=${formValue.model ?? ""}`
    );
  };
  ...
  const showSearchSuggestions = searchInputFocused && searchProductData?.length > 0;
      ...
      <div className="container sm:px-8 px-4">
        ...
        <div className="hidden xl:flex justify-between pb-12">
          ...
          <div className="flex items-center space-x-10">
            <div className={`border-r border-t border-l ${showSearchSuggestions ? "rounded-t-lg" : "border-b rounded-lg"} border-primary-200 xl:w-[45.46rem] 2xl:w-[50rem] relative min-h-[3.375rem] 2xl:min-h-[4rem]`}>
              ...
              {/* search suggestions */}
              <SearchSuggestion showSearchSuggestions={showSearchSuggestions} searchProductData={searchProductData} />
              {/* voice search modal */}
              {showVoiceSearchModal && <SearchByVoice searchText={searchText} setShowVoiceSearchModal={setShowVoiceSearchModal} handleListening={handleListening} isListening={isListening} />}
              {/* image search modal */}
              {device === "desktop" && <SearchByImage showImageSearchModal={showImageSearchModal} setShowImageSearchModal={setShowImageSearchModal} />}
              {/* advance search */}
              <div
                className={`${filterToggle ? "visible" : "hidden"
        ...
        <div className="flex justify-center xl:justify-start items-center space-x-6 xl:-mb-1">
                    ...
                    cate_data.map((item) => (
                      <div key={item?.category_id}>
                        <MenubarSub className="rounded-b-xl">
                          ...
                          <MenubarSubContent
                            className={`ml-1 shadow border-none py-3 px-4 rounded-lg bg-white 2xl:rounded-xl ${item?.sub_categories ? "visible" : "invisible"
                              }`}
                          >
                            {item?.sub_categories &&
      ...
      <div className="px-4 xl:hidden flex justify-between items-center gap-4 py-4 sm:px-8 border-t border-b border-primary-200">
        <div className="">
          <button className="w-6 h-6" onClick={() => setMenuOpen(true)}>
            <Menu size={30} className="-ml-1 mt-[6px] stroke-2" color="#01060de3" />
          </button>
          {/* mobile sidebar */}
          <div
            className={`${menuOpen
        ...
        <div className={`flex-1 min-w-[200px] md:min-w-[400px] border-r border-t border-l ${showSearchSuggestions ? "rounded-t-lg" : "border-b rounded-md"} border-primary-200 p-2 flex items-center justify-between relative`}>
          {/* search input */}
          <div className="flex-1 relative">
            <input
          ...
          {showVoiceSearchModal && <SearchByVoice searchText={searchText} setShowVoiceSearchModal={setShowVoiceSearchModal} handleListening={handleListening} isListening={isListening} />}
          {/* image search modal */}
          {device === "mobile" && <SearchByImage showImageSearchModal={showImageSearchModal} setShowImageSearchModal={setShowImageSearchModal} />}
        </div>
        <div>
          {authToken ? (
            <DropdownMenu>
            ...
            <Link href={"/signin"}>
              <div className="flex items-center min-w-0 gap-x-2">
                ...
                <p className="text-xs text-primary-950 font-medium pb-px leading-[32px]">Sign in</p>
              </div>
            </Link>
          )}
        </div>
